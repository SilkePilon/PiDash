...
export const SavedFlowsModal = memo(({ isOpen, onClose }: SavedFlowsModalProps) => {
  ...
  const loadFlows = useCallback(async () => {
    try {
      ...
      // Ensure all required properties exist before setting the flows
      const validatedFlows = savedFlows.filter((flow): flow is SavedFlow => 
        flow.id !== undefined && 
        typeof flow.id === 'string' && 
        typeof flow.createdAt === 'string' && 
        typeof flow.updatedAt === 'string'
      );
      setFlows(validatedFlows);
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-[999999999] overflow-hidden positon-aboslute pointer-events-auto">
      {/* Backdrop with stronger blur */}
      <div 
        className="absolute inset-0 bg-dark-900/70 backdrop-blur-xl"
        onClick={onClose}
      />
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div 
          className="w-[800px] max-h-[80vh] border border-dark-200 rounded-xl bg-dark-300/90 shadow-xl animate-fadeInScale pointer-events-auto"
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <button
                          ...
                          className="size-7 flex items-center justify-center text-red-400 bg-dark-300 hover:bg-dark-400 rounded-md opacity-0 group-hover:opacity-100 transition-opacity"
                          title="Delete flow"
                        >
